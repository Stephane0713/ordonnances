...
@php
  ...
  $styles = match (true) {
    !$prescription->patient->consent_file => 'bg-gray-200 hover:bg-gray-300',
...
<tr {{ $attributes->merge(['class' => $styles]) }}>
  <td class="px-4 py-2 text-sm">{{ strtoupper($prescription->patient->last_name) }}
    {{ $prescription->patient->first_name }}
  </td>
  <td class="px-4 py-2 text-sm text-center"><x-contact-icon method="{{ $prescription->patient->contact_method }}"
      value="{{ $prescription->patient->contact_value }}" /></td>
  <td class="px-4 py-2 text-sm">{{ $prescription->patient->getSSN() }}</td>
  ...
  <td class="text-right px-4 py-2 text-sm">
    <x-adv-dropdown align="right" width="w-max">
      <x-slot name="trigger">
        <button
          class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
          <div class="ms-1 pointer-events-none ">
